# nf-core/reaptec: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/reaptec/usage](https://nf-co.re/reaptec/usage)
...
```console
nextflow run nf-core/reaptec --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/reaptec releases page](https://github.com/nf-core/reaptec/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
